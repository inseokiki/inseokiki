...
**적용 범위**: 이 원칙은 3GPP 표준 파라미터/수식/절차를 인용하거나 표준 준수 여부를
판단하는 주장에 적용된다. Makefile/빌드 시스템/git 정리/CLAUDE.md·스킬 설정 같은 순수
엔지니어링 작업에는 spec-lookup 서브에이전트 위임이나 표준 문서 확인을 강제하지 않는다
— 이런 작업에서 표준 근거가 필요 없다는 판단 자체가 이 예외에 해당하는지의 기준이다.
(2026-09-03 확인 — 매 사소한 변경마다 spec 확인 절차를 거치는 오버헤드를 줄이기 위함)
