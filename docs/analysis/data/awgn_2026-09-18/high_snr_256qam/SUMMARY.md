...
현재 구현의 8개 대표 설정, 16회 실행, 96개 지표 측정값을 수집했다. 모든 실행이 정상 종료하고 예상 SNR 행 수·유한값·확률 범위 검사를 통과했다.

이 실행은 [기본 69개 설정 측정](../SUMMARY.md)의 256QAM 고 SNR 보완이다.
기존 SCH rate-matching의 교정 전 구현을 측정했다.
[발견된 공백](../../../ldpc_rate_matching_gap.md)을 함께 읽어야 한다.

## 시험 조건

- SNR: [25, 30, 35] dB. 시뮬레이터 입력 SNR이며 Eb/N0로 변환하지 않았다.
...
| Case | 지표 | 25 dB | 35 dB |
|---|---|---:|---:|
| pdsch_coded_256qam | bler | 0 | 0 |
| pdsch_dmrs_256qam_zf | bler | 0.0875 | 0 |
| pusch_tp0_256qam_zf | bler | 0.115 | 0 |
| pusch_tp1_256qam_zf | bler | 0.075 | 0 |
| pdsch_dmrs_256qam_mmse | bler | 0.09 | 0 |
| pusch_tp0_256qam_mmse | bler | 0.1125 | 0 |
| pusch_tp1_256qam_mmse | bler | 0.075 | 0 |
| ber_256qam | ber | 0 | 0 |
...
```bash
python3 PHY/tests/run_awgn_campaign.py --output /tmp/awgn-repeat --trials 200 --seeds 12345 67890 --snr-start 25 --snr-end 35 --snr-step 5 --case pdsch_coded_256qam --case pdsch_dmrs_256qam_zf --case pusch_tp0_256qam_zf --case pusch_tp1_256qam_zf --case pdsch_dmrs_256qam_mmse --case pusch_tp0_256qam_mmse --case pusch_tp1_256qam_mmse --case ber_256qam
```
