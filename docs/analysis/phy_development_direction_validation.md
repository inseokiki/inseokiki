...
## 13. 나머지 P0/P1 항목 상태 갱신 (2026-09-11 문서 정리 세션에서 대조)

이 문서 마지막 갱신(Section 12, 2026-09-02) 이후 완료된 항목과, 이 문서 작성 시점부터
지금까지 계속 열려 있는 항목을 대조했다. 코드 수정은 하지 않고 기존 완료 기록과 현재
코드만 대조했다.

- **P0-2c(다중 코드블록 세그멘테이션)**: 완료(2026-09-03). `docs/analysis/history/2026-09.md`
  의 "P0-2c" 3개 항목(초기+파일럿/HARQ 없는 나머지 35개/HARQ 있는 나머지 14개) 참조.
- **P0-4(Polar 표준 정합화)**: 완료(2026-09-04). 신뢰도 시퀀스(TS 38.212 Table 5.3.1.2-1)
  실제 표 추출 적용, 입력 인터리빙·PC 비트 구현, CA-SCL 디코더(`polar_decode_scl`)로 PBCH/
  PDCCH/PUCCH/main.c 전 채널 연결. `docs/analysis/history/2026-09.md`의 "Polar 코드 표준
  정합화 Phase 4..." 항목 참조.
- **P1-1(TDL을 TR 38.901 TDL-A/B/C/D/E로 교체)**: 여전히 미착수(2026-09-11 코드 대조 —
  `tdl.c`는 그대로 6-tap 근사). `tasks/todo.md` "진행 중"으로 이동.
- **P1-2(빔관리 TDL 공유-스칼라 근사)**: 10.2에서 이미 확인했듯 새 결함이 아니라 코드
  주석 수준에서 이미 명시된 근사 — 추가 조치 불필요, 그대로 둔다.
- **P1-3(RE-domain 전용, 실제 waveform 미통과)**: 여전히 미착수(2026-09-11 확인 — 여전히
  `main.c`의 legacy 경로 1곳만 `ofdm_modulate`/`ofdm_demodulate` 호출). `tasks/todo.md`
  "진행 중"으로 이동.
- **P1-4(TBS가 TS 38.214 §5.1.3.2 표준 절차 아님)**: 여전히 미착수(2026-09-11 코드 대조 —
  `pdsch.c`의 다수 `run_*` 함수가 그대로 `tbsz=(int)(max_dbits*cr)` 직접 곱셈, `N_info`
  양자화·TBS 표 참조 절차 없음). 이번 세션에 새로 `tasks/todo.md` "진행 중"에 추가했다
  (이전에는 어디에도 추적되고 있지 않았음).
