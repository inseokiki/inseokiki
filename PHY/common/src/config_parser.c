...
int config_parser_load(ConfigParser *p, const char *filename) {
    ...
    /* TDL + HARQ */
    kv_str(p, "TDL_MODEL",    "TDL_A",     c->tdlModel,       CFG_STR_MAX);
    c->tdlDsRmsNs    = kv_dbl(p, "TDL_DS_RMS_NS",  30.0);
    c->dopplerHz     = kv_dbl(p, "DOPPLER_HZ",       0.0);
    c->harqMaxRounds = kv_int(p, "HARQ_MAX_ROUNDS",    4);

    /* MIMO */
    c->numRxAnt = kv_int(p, "NUM_RX_ANT", 1);

    /* CSV output */
    c->outputCsv  = kv_int(p, "OUTPUT_CSV",  0);
    kv_str(p, "OUTPUT_FILE", "result.csv", c->outputFile, CFG_STR_MAX);
