 ...
 * Rank-2 프리코더
 *
 * TS 38.214 Table 5.2.2.2.1-6 (codebookMode=1) 원식:
 *
 * W = (1/√(2P)) · [v_l,    v_l';
 *                  φ_n·v_l, −φ_n·v_l']            l' = (l+k1) mod (N1·O1)
 *
 * k1은 Table 5.2.2.2.1-3의 "N1=2, N2=1" 열에서 확정한 값 —
 *   i1_3=0 → k1=0 (l'=l, 같은 빔)
 *   i1_3=1 → k1=O1(=4, l'=l+4, 직교 빔 쌍)
 * n = i2, φ_n = e^{jπi2/2}.
 *
 * (2026-08-31 수정 — 이전 코드는 i1_3=0/1 라벨이 스펙과 반대로 붙어
 * 있었고, "빔쌍" 변형은 스펙에 없는 자체 코피에이징 스킴(φ_{n1}=1 고정,
 * φ_{n2}만 i2에 따라 가변)을 썼다. RI/PMI 선택이 두 변형 전체를 전수
 * 탐색하므로 선택된 코드워드의 물리적 성질 자체는 이전과 동일해 기존
 * BLER 실측치에 영향은 없을 것으로 보이나, 실제 3GPP 코드워드 비트
 * 매핑과는 달랐음. 8-port(codebook_8port.c)에서 이 원식을 먼저 검증한
 * 뒤 동일하게 적용함. 상세는 docs/analysis/history.md 참조.)
 *
 * 직교성 증명 (W[:,0]^H W[:,1] = 0, i1_3=0의 l'=l 케이스 포함):
 *   = (1/(2P))·(v_l^H v_l' + φ_n^*·(−φ_n)·v_l^H v_l')
 *   = (1/(2P))·(1−1)·v_l^H v_l' = 0 ✓  (φ_n 항의 부호가 상쇄 —
 *     v_l ⊥ v_l' 여부와 무관하게 항상 성립)
 *
 * 정규화 증명 (열당 0.5, 총 1 — rank-1과 동일 총 전력):
 *   ||W[:,0]||² = (1/(2P))·(||v_l||²+||v_l||²) = (1/(2P))·2·N1 = N1/P = 0.5
 *   (N1=2, P=4) — 동일하게 ||W[:,1]||²=0.5, 합계 1 ✓
 * ════════════════════════════════════════════════════════════════════════ */
void codebook_type1_sp_4port_rank2(int i1_1, int i1_3, int i2, cx_t W[4][2]) {
    int l  = i1_1;
    int k1 = i1_3 * O1;                       /* Table 5.2.2.2.1-3, N1=2,N2=1: k1=i1,3·O1, i1,3∈{0,1} */
    int lp = (l + k1) % (N1 * O1);
    ...
    double norm = 1.0 / sqrt(2.0 * (double)P);   /* 1/√8 — 기존 0.5/√2와 수치상 동일값 */

    W[0][0] = norm * v0[0];          W[0][1] = norm * v1[0];
    W[1][0] = norm * v0[1];          W[1][1] = norm * v1[1];
    W[2][0] = norm * phi * v0[0];    W[2][1] = norm * (-phi) * v1[0];
    W[3][0] = norm * phi * v0[1];    W[3][1] = norm * (-phi) * v1[1];
}
    ...
    if (rank == 1) {
        ...
        for (int i1_3 = 0; i1_3 <= 1; i1_3++) {
            for (int i1 = 0; i1 < N1 * O1; i1++) {
                for (int i2 = 0; i2 < 4; i2++) {
                    cx_t W[4][2];
                    codebook_type1_sp_4port_rank2(i1, i1_3, i2, W);
                    ...
                    printf("i1=%d  i1_3=%d  i2=%d   %.4f      %.4f      %.2e\n",
                           i1, i1_3, i2, n0, n1, cabs(dot));
                }
            }
        }
 ...
 *   기준  C₁ = log₂(1 + ||H·W||²/N₀)
 *
 * Rank-2 탐색: 64 후보 (i1_3∈{0,1} × i1_1∈{0..7} × i2∈{0..3})
