 ...
 *   W = (1/√8) · [v_l, v_l'; φ_n·v_l, −φ_n·v_l']   (4×2)
 *   l' = (l+k1) mod 8,  k1 = i1_3·O1 (Table 5.2.2.2.1-3, N1=2,N2=1):
 *     i1_3=0 → k1=0  (l'=l, 같은 빔)
 *     i1_3=1 → k1=4  (l'=l+4, 직교 빔 쌍)
 *   n = i2, φ_n = e^{jπi2/2}
 *
 * 직교성: W[:,0]^H W[:,1] = 0  (i1_3 값과 무관하게 수학적으로 보장됨 —
 *   φ_n 항의 부호 상쇄로 성립, l'=l 케이스 포함)
 * 정규화: ||W[:,0]||² = ||W[:,1]||² = 0.5, 합계 1 (rank-1과 동일 총 전력)
 *
 * 파라미터:
 *   i1_1 : 빔 인덱스, 0 ≤ i1_1 ≤ 7
 *   i1_3 : k1 선택 (0 또는 1)
 ...
 * (rank, PMI) 조합을 전체 탐색(96 후보 = rank-1 32 + rank-2 64).
 *
 * 선택 기준:
